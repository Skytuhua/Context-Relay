...
| T06 | Provide a SQLCipher vault and local hybrid search. | [vault implementation](../../crates/core/src/vault.rs), [search implementation](../../crates/core/src/search.rs), [real-model search qualification](semantic-search-2026-09-07.md), [Tasks 1–10 ledger](tasks-1-10.md) | partial | Persistent vectors, resumable indexing, tag indexing, service scheduling and owner-only desktop progress/retry are implemented. Complete production BGE/runtime wiring, failure/reinitialization and real-query performance gates on both supported platforms; exercise Keychain/Credential Manager loss. The prior injected-vector timing did not qualify production semantic search. |

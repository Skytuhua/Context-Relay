...
The [reviewed September14 membership activation and reconstruction component](membership-transfer-activation-2026-09-14.md)
now covers local historical authority, bounded repair, exact-target installation,
independent current activation and stale-reader/revocation denials. Its seven
component requirements are accepted through source d43aced. T17 and RB-CRYPTO
remain partial: hosted/installed lifecycle and the remaining release execution
planes are still required. This does not clear the three current daemon
pairing/sync failures or RB-SYNC.
...
| --- | --- | --- | --- | --- |
| T01 | Preserve public history and align the canonical repository without force-push. | [Tasks 1–10 ledger](tasks-1-10.md), [September14 remote and ancestry checks](../repository-settings.md#canonical-remote-and-preserved-history) | verified | Current remote, upstream, original/bootstrap ancestry and nondiverging PR branch verified at local c56c173 against public main b3d487e / PR30589c0. Repeat exact-ref checks before merge; this does not close the separate governance or release gates. |
...
| T06 | Provide a SQLCipher vault and local hybrid search. | [vault implementation](../../crates/core/src/vault.rs), [search implementation](../../crates/core/src/search.rs), [real-model search qualification](semantic-search-2026-09-07.md), [Tasks 1–10 ledger](tasks-1-10.md) | partial | Windows production BGE/runtime loading, failure/reinitialization and disposable authenticated service checks are recorded in [Windows search packaging](windows-search-packaging-2026-09-07.md). [macOS runtime inputs](../../crates/core/models/onnxruntime-osx-arm64-1.24.2/README.md) are pinned and stage-verified. The guarded macOS loader and final signing/inference pipeline passed native job 103552389709 at cce0772. Physical-app resource discovery and disposable daemon recovery qualification passed native macOS job 103555264457 at b59551f. Complete both-platform installed performance and credential-loss gates. Component and injected-vector timings do not qualify clean-machine acceptance. |
...
| T17 | Complete pairing, recovery, reassociation, revocation, rotation, and deletion. | [pairing](task-17-pairing.md), [enrollment](task-17-recovery-enrollment.md), [fresh recovery core](task-17-fresh-install-recovery-core.md), [reviewed local membership](membership-transfer-activation-2026-09-14.md) | partial | Native Windows/macOS phrase entry and configured production hosted recovery/pairing injection are implemented; [continuation evidence](pr16-release-continuation-2026-09-08.md) includes composed restart, wrong/full safety and logout checks. Complete deployed/installed pairing and recovery, production reassociation/revocation/rotation, and end-to-end deletion/export; guarded account lifecycle is now configured at production startup, with composed and live qualification still open. |
...
| RB-CRYPTO | Release blockers: cryptography and devices. | [Task 16 ledger](task-16.md), [pairing](task-17-pairing.md), [recovery](task-17-fresh-install-recovery-core.md), [reviewed local membership](membership-transfer-activation-2026-09-14.md) | partial | Add physical cross-platform vectors, hosted device lifecycle, revocation cutoff, epoch rotation, and interrupted-rotation proof. |
...
2026-09-09 current-source reconciliation at `0bfdcd4`: T06 and T17 now point to
implemented Windows packaged search and hosted recovery/pairing injection rather
than obsolete missing-code claims. `DaemonConfig::production` only supplies
search resources on Windows; `PinnedModelEmbedder::load_packaged` returns an
initialization error elsewhere. macOS packaged search remains implementation
work. A subsequent lifecycle integration now replaces the unavailable default
at configured production startup with a guarded native service using verified
local workspace material and durable original-login intents. Composed and live
qualification remains open. These source findings preserve the
partial statuses and identify the next implementation gates; they do not close
any installed, hosted, signing, physical or beta requirement.

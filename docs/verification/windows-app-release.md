...
| Semantic memory search | Windows production selects verified adjacent BGE/runtime resources; packaged loading, keyword fallback, failure/reinitialization and authenticated disposable-daemon search are covered in [Windows packaging qualification](windows-search-packaging-2026-09-07.md). Persistent indexing and performance evidence are scoped in [search qualification](semantic-search-2026-09-07.md). | Implement macOS packaged runtime loading and production activation; pass both-platform latency, installed workflows and clean-machine resource qualification. |
...
| Login, hosted sync and device lifecycle | Configured production startup injects hosted Auth, recovery and pairing services using protected device keys. Composed pairing tests cover lost responses, both-daemon restart, wrong/full safety confirmation and local completion after logout; see the [continuation ledger](pr16-release-continuation-2026-09-08.md). Configured startup now also wires guarded account lifecycle using verified workspace material and immutable original-login intents; composed and live qualification remains open. | Complete production sync wiring, composed lifecycle qualification and real OAuth/provider deployment, then verify two-device sync, pairing/recovery, reassociation, revocation/rotation and deletion/export on installed clients. |

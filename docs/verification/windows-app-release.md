...
| Semantic memory search | Configured vaults use pinned BGE with persistent derived vectors and a resumable queue. Background scheduling, immediate owner-only progress, retry, and desktop refresh are implemented and pass disposable service/UI tests. The production daemon still uses legacy token-hash search. See [qualification](semantic-search-2026-09-07.md). | Verify model/runtime packaging, failure/reinitialization and production activation, then pass supported-platform latency and installed workflow gates. |

...
| Semantic memory search | Explicitly configured vaults use pinned BGE with persistent derived vectors, a resumable queue, and title/tag/body indexing. Real paraphrase/lifecycle and cold-request tests pass; the production daemon still uses legacy token-hash search. See [qualification](semantic-search-2026-09-07.md). | Integrate background scheduling and visible progress, verify model/runtime packaging and production activation, then pass supported-platform latency and installed workflow gates. |

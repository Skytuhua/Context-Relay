...
| Semantic memory search | Explicitly configured vaults now use pinned BGE and pass real paraphrase/lifecycle tests; the production daemon still uses legacy token-hash search. See [qualification](semantic-search-2026-09-07.md). | Complete responsive cold indexing, tag-search preservation, verified model/runtime packaging and production activation, then pass supported-platform latency and installed workflow gates. |

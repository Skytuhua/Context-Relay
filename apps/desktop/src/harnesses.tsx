...
import type { HarnessId, HarnessParams, PlanId, ProjectIdentity, SetupPlan, WireNativeValue } from './bindings';
import { type HarnessGateway, validateHarnessPlan } from './harness-gateway';
...
export function HarnessesScreen({ gateway, projects, active = true }: { gateway: HarnessGateway; projects: ProjectIdentity[]; active?: boolean }) {
  ...
  const matching = review !== null && review.params.projectId === project?.projectId &&
    review.params.harness === harness && review.params.hermesProfile === (harness === 'hermes' ? profile.trim() : null);
  ...
  async function previewSetup() {
    ...
    const params: HarnessParams = { harness, projectId: project.projectId, hermesProfile: harness === 'hermes' ? profile.trim() : null };
    try {
      ...
      if (mounted.current && revision === generation.current) {
        setError('Setup preview could not be loaded. Check the local service, installed harness and project path, then request a new preview.');
      }
    } finally { finish(); }
  }
  ...
  return (
    <section className="screen-content" aria-label="Harness setup" style={{ overflowWrap: 'anywhere', minWidth: 0 }}>
      <form className="capture-form" aria-label="Preview harness setup" onSubmit={(event) => { event.preventDefault(); void previewSetup(); }}>
        <h2>Set up a local harness</h2>
        <p>Preview discovers the installed harness and prepares a plan for your review. Setup changes only run after approval.</p>
        {projects.length === 0 && <p>Add a project in Projects before previewing setup.</p>}
        <div className="field">
          <label htmlFor="harness-project">Project</label>
          <select id="harness-project" value={projectId} disabled={busy === 'apply' || busy === 'rollback'} onChange={(event) => { clearReview(); setProjectId(event.target.value); }}>
        ...
        <button className="primary-action" type="submit" disabled={!!busy || !project || (harness === 'hermes' && !profile.trim())}>{busy === 'preview' ? 'Previewing setup…' : 'Preview setup'}</button>
      </form>
      {error && <p className="form-error" role="alert">{error}</p>}
      {notice && <p className="notice" role="status">{notice}</p>}
      {busy && <p role="status">{busy === 'preview' ? 'Discovering the installed harness…' : busy === 'apply' ? 'Applying the reviewed plan…' : 'Rolling back the confirmed plan…'}</p>}
      {review && <section className="record-card" aria-labelledby="harness-review-title">
        <h2 id="harness-review-title">Review setup plan</h2>
        <p>{label(review)}</p>
...
function PlanDetails({ plan }: { plan: SetupPlan }) {
  return <>
    ...
    <p>Approval class: {plan.approvalClass}</p>
    ...
    {plan.semanticChanges.length ? <ul>{plan.semanticChanges.map((change, index) => <li key={index}><strong>{change.class}: {change.target}</strong><p>{change.summary}</p></li>)}</ul> : <p>No semantic changes.</p>}
    ...
    </li>)}</ul> : <p>No package artifacts.</p>}
    <details>
      <summary>Identity and digest details</summary>

...
it('connects the newly added project even when another project already exists', async () => {
  ...
  fireEvent.click(await screen.findByRole('button', { name: 'Open Harnesses' }));
...
it('keeps an acknowledged new context visible when an older list arrives', async () => {
  ...
  let savedAcknowledged = false;
  const memories = vi.fn(() => savedAcknowledged ? Promise.resolve([saved, existing]) : new Promise<MemoryRecord[]>(resolve => { finishList = resolve; }));
  render(<App gateway={{ ...gateway, projects: async () => [project], memories, createMemory: async () => { savedAcknowledged = true; return saved; } }} />);
  await screen.findByText('Ready on this computer');
  fireEvent.click(screen.getByRole('button', { name: 'Context' }));
...
it('keeps an unsaved context draft and explains a failed save without exposing native details', async () => {
  ...
  await screen.findByRole('button', { name: 'Open Harnesses' });
  fireEvent.click(screen.getByRole('button', { name: 'Context' }));
  await act(async () => {});
  fireEvent.click(screen.getByRole('button', { name: 'Add context' }));
  const form = screen.getByRole('form', { name: 'New context' });

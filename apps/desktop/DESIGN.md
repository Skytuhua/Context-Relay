...
- `.context-tabs` contains buttons with `aria-selected="true"` for the current tab; application code owns tab keyboard behavior and associated panels.
...
- `.onboarding-shell` contains `.onboarding-rail` and `.onboarding-content`. Put the five progress items in the rail's ordered list and mark the current item with `aria-current="step"`. Completed items may use `data-complete="true"`. `.onboarding-footer` remains at the bottom when space permits, then flows naturally when content grows.
...
At 1024px, editor columns stack. At 736px, labeled sidebar navigation becomes a wrapping top region; dashboard and device/pairing columns stack, and setup progress moves above its content. Minimums are used instead of fixed heights so text enlargement can increase layout height. Long pages scroll normally, including smaller desktop windows. Do not clip overflowing content to conceal layout problems.

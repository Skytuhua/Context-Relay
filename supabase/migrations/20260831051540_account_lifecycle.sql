...
begin
  ...
  -- Predicates evaluated before FOR SHARE waits can expire while blocked.
  return exists (
    select 1 from auth.sessions as session
    where session.id = p_session_id and session.user_id = p_auth_user_id
...
begin
  ...
  if receipt is not null then
    return context_relay_private.account_deletion_projection(account_id);
  end if;

...
test("lifecycle SQL declares bound receipts and a current-state replay path", async () => {
  const sql = await lifecycleMigration();
  ...
  for (const name of ["service_begin_account_deletion_for_session", "service_cancel_account_deletion_for_session"]) {
    ...
    assert.match(body, /if receipt is not null then\s*return context_relay_private\.account_deletion_projection\(account_id\);/i);
  }
});

...
fn schema_33_upgrade_never_invents_lifecycle_authority() {
    ...
    raw.execute_batch("DROP TABLE IF EXISTS revocation_control_history; DROP TABLE IF EXISTS device_revocation_intents; DROP TABLE IF EXISTS candidate_aliases; DROP TABLE account_lifecycle_intents; PRAGMA user_version=33;")
        .unwrap();
    drop(raw);

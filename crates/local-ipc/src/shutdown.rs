...
mod tests {
    ...
    #[tokio::test]
    async fn authenticated_shutdown_accepts_the_previous_1_15_installer() {
        assert_shutdown_waits_for_exit("legacy-ack-1-15").await;
    }

    #[tokio::test]
    async fn authenticated_shutdown_accepts_the_previous_1_16_installer() {
        assert_shutdown_waits_for_exit("legacy-ack-1-16").await;
    }

    #[tokio::test]
    async fn ordinary_client_still_rejects_the_previous_1_16_installer() {
        let fixture = Fixture::start("legacy-client-1-16", false).await;
    ...
    #[tokio::test]
    async fn ordinary_client_still_rejects_the_previous_1_15_installer() {
        let fixture = Fixture::start("legacy-client-1-15", false).await;
        let stream = connect(&fixture.runtime).await.unwrap();
        let result =
            crate::Client::from_stream(stream, ClientRole::Desktop, &token().unwrap()).await;
        assert!(matches!(result, Err(IpcError::ProtocolVersionUnsupported)));
    ...
    #[tokio::test]
    async fn authenticated_shutdown_accepts_the_previous_1_14_installer() {
        assert_shutdown_waits_for_exit("legacy-ack-1-14").await;
    }

    #[tokio::test]
    async fn ordinary_client_still_rejects_the_previous_1_14_installer() {
        let fixture = Fixture::start("legacy-client-1-14", false).await;
        let stream = connect(&fixture.runtime).await.unwrap();
        let result =
            crate::Client::from_stream(stream, ClientRole::Desktop, &token().unwrap()).await;
        assert!(matches!(result, Err(IpcError::ProtocolVersionUnsupported)));
    ...
    #[tokio::test]
    async fn authenticated_shutdown_accepts_the_previous_1_13_installer() {
        assert_shutdown_waits_for_exit("legacy-ack-1-13").await;
    }

    #[tokio::test]
    async fn ordinary_client_still_rejects_the_previous_1_13_installer() {
        let fixture = Fixture::start("legacy-client-1-13", false).await;
        let stream = connect(&fixture.runtime).await.unwrap();
        let result =
            crate::Client::from_stream(stream, ClientRole::Desktop, &token().unwrap()).await;
        assert!(matches!(result, Err(IpcError::ProtocolVersionUnsupported)));
    ...
    #[tokio::test]
    async fn ordinary_client_still_rejects_the_previous_1_11_installer() {
        let fixture = Fixture::start("legacy-client-1-11", false).await;
        let stream = connect(&fixture.runtime).await.unwrap();
        let result =
            crate::Client::from_stream(stream, ClientRole::Desktop, &token().unwrap()).await;
        assert!(matches!(result, Err(IpcError::ProtocolVersionUnsupported)));
    ...
    async fn legacy_server(mut stream: ConnectedStream, root: &Path, mode: &str) {
        let protocol = ProtocolVersion {
            ...
            minor: match mode {
                ...
                "legacy-ack-1-13" | "legacy-client-1-13" => 13,
                "legacy-ack-1-14" | "legacy-client-1-14" => 14,
                "legacy-ack-1-15" | "legacy-client-1-15" => 15,
                "legacy-ack-1-16" | "legacy-client-1-16" => 16,
                _ => 4,
            },
        };
        ...
        match mode {
            ...
            | "legacy-ack-1-12" | "legacy-ack-1-13" | "legacy-ack-1-14" | "legacy-ack-1-15"
            | "legacy-ack-1-16" => {}
